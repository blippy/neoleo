...
(define calc-parser
  (lalr-parser
   ...
   ;; --- token definitions
   (ID NUM = LPAREN RPAREN NEWLINE COMMA IFS
       (left: + -)
       (left: * /)
       (nonassoc: uminus))
   ...
   (if-statement
    (IFS LPAREN expr COMMA expr COMMA expr RPAREN) : (if (not (eq? $3 0)) $5 $7))
...
(define my-lexer (make-lexer errorp))
...
(define (!eof? obj) (not (eof-object? obj)))

(define (more? obj) (not (eq? obj '*eoi*)))

(while (more? (set!! toke (my-lexer)))
       (write toke)
       #t)
