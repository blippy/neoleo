      ...
      else
	{
	  ...
	  if (!Global->alarm_active)
	  ...
	  else
	    {
	      struct timeval tv;
	      tv.tv_sec =
		Global->alarm_seconds == 1 ? 1 : Global->alarm_seconds / 2;
	      tv.tv_usec = 0;
...
get_argument (char *prompt, struct prompt_style *style)
{
  ...
  if (the_cmd_frame->cmd->init_code && the_cmd_frame->cmd->init_code[cur_arg])
    {
      char *init_code =
	expand_prompt (the_cmd_frame->cmd->init_code[cur_arg]);
      struct rng rng;
  ...
  while (1)
	...
	tryagain:
	  ...
	  switch (ch)
		...
		for (cur_cmd =
		     &the_funcs[cur_vector][0]; cur_cmd->func_name; cur_cmd++)
		  if (!strincmp
		      ((char
			*)
		       (rmac->mac_exe),
		       cur_cmd->func_name,
		       len) && cur_cmd->func_name[len] == '\0')
		    {
		      cur_chr = '\0';
		      goto out;
		    }
	      ...
	      if (*ptr == ' ')
		{
		  ...
		  if (!cur_cmd->func_args)
		    {
		      io_error_msg
			("Ignoring extra operand to %s", cur_cmd->func_name);
		      while (*ptr && *ptr != '}')
			ptr++;
		      if (*ptr == '}')
			ptr++;
		  ...
		  else if (cur_cmd->func_args[0][0] == '+')
		    {
		      ...
		      setn_line (&raw_prefix, (char *) start, ptr - start);
		      if (*ptr == '}')
			ptr++;
		    }
		  else
      ...
      while (1)
	{
	  ...
	  if (key->vector < 0)
		...
		{
		  cur_keymap = the_maps[cur_keymap]->map_next->id;
		}
	      else
		{
		  cur_vector = 0;
	  ...
	  switch (*prompt)
		...
		switch (type)
		  ...
		  default:
		    ...
		    io_error_msg
		      ("func_args bug for %s", the_cmd_frame->cmd->func_name);
		  }
		...
		if (get_argument
		    (prompt, (want_keyseq ? &keyseq_style : &mode_style)))
		  //goto new_cycle;
		  return true;	// state machine
		goto next_arg;
	      }
		...
		if (*prompt == '?')
		  {		/* Command wants to know if prefix provided */
		    ...
		    the_cmd_arg.style = &int_constant_style;
		    init_arg_text
		      (&the_cmd_arg, the_cmd_arg.val.integer ? "1" : "0");
		  }
		else
		  {
...
#define interactive_mode (!rmac || iscmd)
...
#endif
		if ((*prompt != 'R'
		     && interactive_mode && mark_is_set) || *prompt == '@')
		  {
			...
			init_arg_text
			  (&the_cmd_arg, range_name (&the_cmd_arg.val.range));
		      }
		    goto next_arg;
		  }
		else
		...
		if (((!cp
		      ...
		      LCK_DEF)
		     && (default_lock ==
			 LCK_LCK)) || (cp && GET_LCK (cp) == LCK_LCK))
		  {
		    io_error_msg
		      ("Cell %s is locked", cell_name (curow, cucol));
		    pop_unfinished_command ();
		    //goto new_cycle;
		    return true;	// state machine
		  }
...
inner_command_loop (int state, int iscmd)
{
  ...
  while (1)
	...
	resume_getting_arguments:
	  ...
	  if (resume_getting_arguments_loop (interactive_mode, iscmd))
	    goto new_cycle;
...
page_info (int rep)
{
  if (rep < 0)
	...
	((next >= the_cmd_arg.prompt_info->len)
	 ? MAX (0, (the_cmd_arg.prompt_info->len - vis_lines)) : next);
    }
  else
    io_error_msg ("No info to page.");
}
